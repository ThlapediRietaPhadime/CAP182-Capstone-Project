...
| **Assumptions** |1. Client records can be reliably linked across the demographic, marketing, behavioural and account/funding datasets using a common Client ID. 2. The available historical data contains sufficient information about client behaviour before the first deposit outcome to train a predictive model.3 The available demographic, acquisition onboarding and behavioural data accurately represents client activity 4.The definition of activation based on whether a client makes a first deposit, can be agreed upon with STADIOEquities|
